...
def build_model(input_shape):
    ...
    f = forward_tcn(inputs)

    # b = f（ Attention(Q=f, K=f, V=f)）
    att = layers.MultiHeadAttention(num_heads=1, key_dim=16)(query=f, value=f, key=f)

    # concat f ，att
    x_cat = layers.Concatenate()([f, att])
    x_pool = layers.GlobalAveragePooling1D()(x_cat)
